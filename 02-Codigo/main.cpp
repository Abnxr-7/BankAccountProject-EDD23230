	...
	while (true) {
		...
		if (tecla == 224) { // Tecla especial
			...
			case 12: // Eliminar Cuenta
			{
				if (banco.getListaPersonas() == nullptr) {
					system("cls");
					std::cout << "No hay cuentas registradas para eliminar.\n";
					system("pause");
					break;
				}

				std::string opcionesEliminar[] = { "Por cedula", "Por numero de cuenta", "Cancelar" };
				int numOpcionesEliminar = sizeof(opcionesEliminar) / sizeof(opcionesEliminar[0]);
				int seleccionEliminar = 0;
				...
				while (true) {
					...
					if (tecla == 224) {
						tecla = _getch();
						if (tecla == 72) seleccionEliminar = (seleccionEliminar - 1 + numOpcionesEliminar) % numOpcionesEliminar;
						else if (tecla == 80) seleccionEliminar = (seleccionEliminar + 1) % numOpcionesEliminar;
					}
					else if (tecla == 13) break;
					else if (tecla == 27) { seleccionEliminar = 2; break; }
				}

				if (seleccionEliminar == 2) break; // Cancelar
				...
				if (seleccionEliminar == 0) { // Por cédula
					...
					while (true) {
						...
						else if (tecla == 13 && digitos == 10) {
							std::cout << std::endl;
							break;
						}
						else if (tecla == 27) { cedula.clear(); break; }
					}
					if (!cedula.empty()) {
						banco.eliminarCuentaPorCedula(cedula);
					}
				}
					...
					while (true) {
						...
						else if (tecla == 8 && !numCuenta.empty()) {
							numCuenta.pop_back();
							std::cout << "\b \b";
						}
						else if (tecla == 13 && !numCuenta.empty()) {
							std::cout << std::endl;
							break;
						}
						else if (tecla == 27) { numCuenta.clear(); break; }
					}
					if (!numCuenta.empty()) {
						banco.eliminarCuentaPorNumCuenta(numCuenta);
					}
				}
				system("pause");
				break;
