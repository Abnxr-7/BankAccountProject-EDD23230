...
//Metodo para eliminar una cuenta por cedula
void Banco::eliminarCuentaPorCedula(const std::string& cedula) {
	if (!listaPersonas) {
	...
	// Eliminar backups asociados (en escritorio)
	std::string rutaEscritorio = obtenerRutaEscritorio();
	std::string nombreArchivo = cedula + ".bak";
	std::string rutaBackup = rutaEscritorio + nombreArchivo;
	std::remove(rutaBackup.c_str());

	// Eliminar archivos hash si existen
	std::string rutaHash = rutaBackup + ".hash";
	std::remove(rutaHash.c_str());

	// Eliminar archivos cifrados si existen
	std::string rutaBin = rutaEscritorio + cedula + ".bin";
	std::remove(rutaBin.c_str());
	...
	if (anterior) {
		...
		listaPersonas = actual->siguiente;
	}
	delete actual;

	std::cout << "Cuenta y datos asociados eliminados correctamente.\n";
}

//Eliminar cuenta por numero de cuenta
void Banco::eliminarCuentaPorNumCuenta(const std::string& numeroCuenta) {
	if (!listaPersonas) {
		std::cout << "No hay personas registradas.\n";
		return;
	}
	...
	NodoPersona* anterior = nullptr;
	bool encontrado = false;
	std::string cedulaEncontrada;

	while (actual) {
		if (actual->persona && actual->persona->isValidInstance()) {
			// Buscar en cuentas de ahorro
			CuentaAhorros* ca = actual->persona->getCabezaAhorros();
			while (ca) {
				if (ca->getCuentaAhorros() && ca->getNumeroCuenta() == numeroCuenta) {
					encontrado = true;
					cedulaEncontrada = actual->persona->getCedula();
					break;
				}
				ca = ca->getSiguiente();
			}
			// Buscar en cuentas corrientes solo si no se encontró en ahorros
			if (!encontrado) {
				CuentaCorriente* cc = actual->persona->getCabezaCorriente();
				while (cc) {
					if (cc->getCuentaCorriente() && cc->getNumeroCuenta() == numeroCuenta) {
						encontrado = true;
						cedulaEncontrada = actual->persona->getCedula();
						break;
					}
	...
	// Eliminar backups asociados (en escritorio)
	std::string rutaEscritorio = obtenerRutaEscritorio();
	std::string nombreArchivo = cedulaEncontrada + ".bak";
	std::string rutaBackup = rutaEscritorio + nombreArchivo;
	std::remove(rutaBackup.c_str());
	...
	// Eliminar archivos cifrados si existen
	std::string rutaBin = rutaEscritorio + cedulaEncontrada + ".bin";
	std::remove(rutaBin.c_str());
