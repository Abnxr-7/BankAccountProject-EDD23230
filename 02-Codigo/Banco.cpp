...
void Banco::cargarCuentasDesdeArchivo(const std::string& nombreArchivo) {
    ...
    while (std::getline(archivo, linea)) {
        ...
        if (linea == "===PERSONA_INICIO===") {
            ...
            CuentaAhorros* nuevaCuenta = new CuentaAhorros(numCuenta, saldo, fechaApertura, estado, 5, 0);
            NodoCuentaAhorros* nodo = new NodoCuentaAhorros(nuevaCuenta);

            // Insertar en la lista de cuentas de ahorro
            nodo->siguiente = personaActual->getCabezaAhorros();
            if (personaActual->getCabezaAhorros() != nullptr) {
                personaActual->getCabezaAhorros()->anterior = nodo;
            }
            ...
            nodo->siguiente = personaActual->getCabezaCorriente();
            if (personaActual->getCabezaCorriente() != nullptr) {
                personaActual->getCabezaCorriente()->anterior = nodo;
            }
    ...
    if (seleccionBusqueda == 3) { // Por cedula
        ...
        while (actual) {
            if (actual->persona && actual->persona->isValidInstance() && 
                ...
                NodoCuentaAhorros* nodoAhorro = actual->persona->getCabezaAhorros();
                ...
                NodoCuentaCorriente* nodoCorriente = actual->persona->getCabezaCorriente();
    ...
    if (seleccion == 4) { // Si es búsqueda por cédula, ya está implementado correctamente
        ...
        while (actual) {
            if (actual->persona && actual->persona->isValidInstance() &&
                ...
                std::cout << "\n===== CUENTAS DE AHORRO =====\n";
                NodoCuentaAhorros* ahorros = actual->persona->getCabezaAhorros();
                int contadorAhorros = 0;
                if (!ahorros) {
                ...
                std::cout << "\n===== CUENTAS CORRIENTES =====\n";
                NodoCuentaCorriente* corrientes = actual->persona->getCabezaCorriente();
                int contadorCorrientes = 0;
                if (!corrientes) {
...
void Banco::realizarTransferencia() {
    ...
    while (!cuentaOrigenEncontrada) {
        ...
        while (actual && !cuentaOrigenEncontrada) {
            ...
            NodoCuentaAhorros* nodoAhorros = actual->persona->getCabezaAhorros();
